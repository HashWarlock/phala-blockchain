...
use phala_types::messaging::OffChainAuctionCommand;

type Command = OffChainAuctionCommand;
...
impl OffChainAuctionBot {
    pub fn new() -> Self {
        OffChainAuctionBot {
            owner: Default::default(),
            bot_token: Default::default(),
            chat_id: Default::default(),
            nft_price: Default::default(),
        }
    }
}
...
/// The current NFT bid price
#[derive(Deserialize, Serialize, Debug)]
struct NftPrice {
    #[serde(rename(deserialize = "KSM"))]
    ksm: String,
}
...
impl contracts::NativeContract for OffChainAuctionBot {
    type Cmd = Command;
    type QReq = Request;
    ...
    fn handle_command(
        ...
        match cmd {
            ...
            Command::SendAuctionBid { bidder, ksm, nft_id } => {
                if sender != alice && sender != self.owner {
                    return Err(TransactionError::BadOrigin);
                }
                ...
                let nft_price = self.nft_price.clone();
                self.nft_price = ksm;
                ...
                let task = AsyncSideTask::spawn(
                    ...
                    async move {
                        ...
                        log::info!("Side task starts to get NFT price");
                        ...
                        log::info!("Side task got NFT price: {}", result);


                        let text = format!("NFT {} current bid price: {} KSM by Owner: {}", nft_id, nft_price, bidder);
                        let uri = format!(
                        ...
                        log::info!("Side task sent NFT price: {}", result);
                        result
                    },
                    |_result, _context| {
    ...
    fn handle_query(
        ...
        match req {
            ...
            Request::QueryNftPrice => {
                Ok(Response::NftPrice(self.nft_price.clone()))
            }
        }
    }
}
